...
settingsImageSequence_beh; % Load all the settings from the file
...
% Set training day.
prefs.day = 1;%str2num(input('Please enter the training day here [1, 2, 3, 4, 5]: ', 's'));
...
% if exist(fullfile(savedir, ['sub' num2str(prefs.subID) '_test_gen_day4.mat']), 'file') == 2
...
%     end
%     clear ch ch2
% elseif exist(fullfile(savedir, ['sub' num2str(prefs.subID) '_test_gen_day3.mat']), 'file') == 2
%     prefs.day = 4; flag = 0;
% elseif exist(fullfile(savedir, ['sub' num2str(prefs.subID) '_test_gen_day2.mat']), 'file') == 2
%     prefs.day = 3; flag = 0;
% elseif exist(fullfile(savedir, ['sub' num2str(prefs.subID) '_test_gen_day1.mat']), 'file') == 2
%     prefs.day = 2; flag = 0;
% else
%     prefs.day = 1; flag = 0;
% end
...
if prefs.group == 1
    td_imageFolder = fullfile(localdir, 'stimuli/hand_symbols_all_group1/');
elseif prefs.group == 2
    td_imageFolder = fullfile(localdir, 'stimuli/hand_symbols_all_group2/');
elseif prefs.group == 3
    td_imageFolder = fullfile(localdir, 'stimuli/hand_symbols_all_group3/');
end
...
t_imgList = dir(fullfile(td_imageFolder,'HS*.bmp'));
d_imgList = dir(fullfile(td_imageFolder,'HD*.bmp'));
idnow = randi(5);
if idnow == 1
...
% if issueflag
    outputfile = fopen([savedir '/bcl_sub' num2str(prefs.subID) '_test_gen_' datestr(now,'mm-dd-yyyy_HH-MM') '.txt'],'a');
% else
%     outputfile = fopen([savedir '/sub' num2str(prefs.subID) '_test_gen_day' num2str(prefs.day) '.txt'],'a');
% end
...
% if issueflag
    save(fullfile(savedir, ['bcl_sub' num2str(prefs.subID) '_test_gen_' datestr(now,'mm-dd-yyyy_HH-MM') '.mat']))
% else
%     save(fullfile(savedir, ['sub' num2str(prefs.subID) '_test_gen_day' num2str(prefs.day) '.mat']))
% end
Screen('FillRect', prefs.w1, prefs.backColor);
...
% copyfile(fullfile(savedir, ['sub' num2str(prefs.subID) '_test_gen*.mat']), fullfile(localdir, 'data'))
% copyfile(fullfile(savedir, ['sub' num2str(prefs.subID) '_test_gen*.txt']), fullfile(localdir, 'data'))
