function [] = bcl_test_practice(subID)
...
% if issueflag
    outputfile = fopen([savedir '/bcl_sub' num2str(prefs.subID) '_practice_' datestr(now,'mm-dd-yyyy_HH-MM') '.txt'],'a');
% else
...
randomizedTrials = randperm(length(td_imgList), nTrials);
...
% if issueflag
    save(fullfile(savedir, ['bcl_sub' num2str(prefs.subID) '_practice_' datestr(now,'mm-dd-yyyy_HH-MM') '.mat']))
% else
